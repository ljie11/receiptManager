(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('receiptManagerApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-receiptManagerApp-error');
                var entityKey = httpResponse.headers('X-receiptManagerApp-params');
                if (errorHeader) {
