...
class ReceiptGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Receipt entity")
        ...
        .repeat(2) {
            ...
            .post("/api/receipts")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "category":null, "created_by":"0", "created_date":"2020-01-01T00:00:00.000Z", "total":"SAMPLE_TEXT", "claimed_amount":null}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_receipt_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
